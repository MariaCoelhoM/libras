...
def build_dataset(dataset_dir, model_path, annotations=None, filename_regex=None, quiet=False, failures_log=None):
    X, y = [], []
    failed = 0
...
def main():
    ...
    parser.add_argument("--filename_regex", default=None, help="Regex com 1 grupo de captura para extrair a classe do nome do arquivo (necessario para datasets com pasta unica e sem CSV, ex.: MINDS-Libras: '^\\d+([A-Za-zÀ-ÿ]+)Sinalizador\\d+-\\d+\\.mp4$'). Nao use junto com --annotations_csv.")
    ...
    if args.zip_path:
        X, y = build_dataset_from_zip(args.zip_path, args.model_path, annotations=annotations, filename_regex=filename_regex, quiet=args.quiet, failures_log=args.failures_log)
    else:
        X, y = build_dataset(args.dataset_dir, args.model_path, annotations=annotations, filename_regex=filename_regex, quiet=args.quiet, failures_log=args.failures_log)
